...
import { AiInDigitalPaymentsForm } from '@/[lang]/components/forms/ai-in-digital-payments-form'
...
export default async function AiInDigitalPayments({ params }: any) {
  const res = await getDigitalSubUrlField('ai-in-digital-payments', params.lang)
  const resSubfields = await getChallangeRoundSubUrlField('ai-in-digital-payments', params.lang)
    ...
    <main className='opicpage'>
      <section className='section-py-md'>
        <TopHeaderSection
          heading={
            res?.heading ||
            (isUrdu ? 'ڈیجیٹل ادائیگیوں میں مصنوعی ذہانت' : 'Artificial Intelligence in Digital Payments')
          }
          subheading={res?.subheading || ''}
          btntitle={isUrdu ? 'درخواست دیں' : 'Apply Now'}
          btnUrl={'#aiDigitalPayments'}
          lang={params.lang}
          btnstyle={'butn-solid'}
          secondaryButton={true}
          proposalTemplate='https://krndevelop.wpenginepowered.com/wp-content/uploads/2025/10/TORs-Call-for-Application-AI-in-Digital-Payments-Final.pdf'
      ...
      <Keydates
        ...
        TermsOfRefference='https://krndevelop.wpenginepowered.com/wp-content/uploads/2025/10/TORs-Call-for-Application-AI-in-Digital-Payments-Final.pdf'
      />
        ...
        <div className='container'>
          <div className='row'>
            <div className='col-md-10 mx-auto'>
              <h2 id='aiDigitalPayments' className='mb-5 text-center'>
                {'Apply Now'}
              </h2>
              <AiInDigitalPaymentsForm lang={params.lang} />
            </div>
          </div>
        </div>
      </section>
