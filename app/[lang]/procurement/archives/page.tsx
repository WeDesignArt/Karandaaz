...
export default function ProcurementArchive({ params }: any) {
  ...
  // Update page to start ie 1 on offset = 0 bc doesn't happen automatically
  useEffect(() => {
    if (offset == 0) {
      setPage(1)
    }
        ...
        <div className='container'>
          <div className='row'>
            <div className='col-md-10 text-center mx-auto'>
              ...
              <h2 className='pb-2 pb-md-2'>{isUrdu ? 'محفوظ شدہ مواقع' : 'Archive Opportunities'}</h2>
            </div>
            <div className='col-md-9 text-center mx-auto'>
              <h4 className='tx-grey50'>
                {isUrdu ? 'یہ مواقع اب بند ہو چکے ہیں' : 'These opportunities are now closed'}
              </h4>
            </div>
          </div>
        </div>
          ...
          <div className='row gy-4'>
            {opportunities.length != 0 ? (
              opportunities?.map((opportunity: any) => {
                return (
                  <div className='col-md-6'>
                    <JobPositionCard title={opportunity?.title} link={`/procurement/archives/${opportunity?.slug}`} />
                  </div>
                )
              })
            ) : (
