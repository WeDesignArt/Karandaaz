...
import HighlightsCard from '@/[lang]/components/core/cards/highlights-card'
import './page.css'

export default async function PrivateSectorEngagements({ params }: any) {
  const res = await getDigitalSubUrlField('research-challengerounds-kdc', params.lang)
  const resSubfields = await getChallangeRoundSubUrlField('research-challengerounds-kdc', params.lang)
  ...
  return (
    <main className='kdcpage'>
      <section className='section-py-md'>
        <TopHeaderSection
          heading={res?.heading}
          subheading={res?.subheading}
          btntitle={res?.btntitle}
          btnUrl={res?.termSheet?.node?.mediaItemUrl}
          lang={params.lang}
          btnstyle={'butn-solid'}
          secondaryButton={true}
          proposalTemplate='#round1'
          proposalTemplateText='Explore Highlights from Round 1'
        />
      ...
      {resSubfields?.videoSection?.videoEditor && (
        ...
        <div className='container'>
          <div className='row'>
            <div className='col-md-12 text-center'>
              <h2 className='mb-3 mb-md-5'>{resSubfields?.highlights?.title}</h2>
              <div
                className='mb-3 mb-md-5'
                dangerouslySetInnerHTML={{ __html: resSubfields?.highlights?.description }}
              ></div>
            </div>
          </div>
          <div className='row'>
            <div className='col-md-6 mb-5 mb-md-0'>
              <HighlightsCard
                title={resSubfields?.highlights?.card1?.title}
                imgLink={resSubfields?.highlights?.card1?.image?.node?.mediaItemUrl}
                link={resSubfields?.highlights?.card1?.link?.url}
                description={resSubfields?.highlights?.card1?.description}
              />
            </div>
            <div className='col-md-6'>
              <HighlightsCard
                title={resSubfields?.highlights?.card2?.title}
                imgLink={resSubfields?.highlights?.card2?.image?.node?.mediaItemUrl}
                link={resSubfields?.highlights?.card2?.link?.url}
                description={resSubfields?.highlights?.card2?.description}
              />
            </div>
          </div>
        </div>
