...
import { researchChallengeRoundsUrl } from '../../../../utils/urls'
...
export default async function PrivateSectorEngagements({ params }: any) {
  const res = await getDigitalSubUrlField('research-challengerounds', params.lang)
            ...
            {Object.keys(digitalSection).map((key, index) => {
              return (
                <DirectionalSection
                  ...
                  buttonLink={
                    isUrdu
                      ? `/${params.lang}${researchChallengeRoundsUrl}/${digitalSection[key]?.buttonLinkSlug}`
                      : `${researchChallengeRoundsUrl}${digitalSection[key]?.buttonLinkSlug}`
                  }
